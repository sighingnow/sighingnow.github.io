...
但在最坏情形下，根据Simon Peyton-Jones的演讲[Programming in the Age of Concurrency: Software Transactional Memory](http://channel9.msdn.com/ShowPost.aspx?PostID=231495)，性能损失也不会超过两倍。理论上，n 个并发事务(transactions)的所需要的时间和空间都为O(n)。

除了性能上的优势，STM也使得多线程程序变得更加容易编写和维护。Lock-based programming 存在很多众所周知的问题(引自[Wikipedia](https://en.wikipedia.org/wiki/Software_transactional_memory))：
...
接下来，我们将通过使用STM来解决[Dining Philosopher Problem](https://en.wikipedia.org/wiki/Dining_philosophers_problem)来演示STM的使用。
...
我们会发现输出显得很乱，原因就在于多个线程同时进行IO输出，为了解决这个问题，我们需要使用STM创建一个IO Buffer：
...
完整程序：[Philosopher.hs]({{site.url}}/sourcecode/Philosopher.hs)
