...
对于一个给定的有向无环图，定义关于图的每个顶点的Sprague-Garundy函数 $g$ 如下：$g(x)=mex\{ g(y) | \textit{ y is x's successor.} \}$。
...
是 $k$ 时，我们可以把它变成 $0$、变成 $1$、$\dots$、变成 $k-1$，但绝对不能保持 $k$ 不变。不知道你能不能根据这个联想到Nim游戏，Nim游戏的规则就是：每次
